...
export default {
  data() {
    let BASE_API = 'http://192.168.248.136'
    ...
    return {
      ...
      uploadUrl: BASE_API + '/web/oss/file/upload', //文件上传地址
    }
  },
  ...
  methods: {
    getUserInfo() {
      this.$axios
        .$get('/web/core/borrower/auth/getBorrowerStatus')
        .then((response) => {
          this.borrowerStatus = response.data

          if (this.borrowerStatus === 0 || this.borrowerStatus === -1) {
            //未认证
            this.active = 1
            this.initSelected()
    ...
    initSelected() {
      //学历列表
      this.$axios
        .$get('/web/core/dict/findByDictCode/education')
        .then((response) => {
      ...
      //行业列表
      this.$axios
        .$get('/web/core/dict/findByDictCode/industry')
        .then((response) => {
      ...
      //收入列表
      this.$axios
        .$get('/web/core/dict/findByDictCode/income')
        .then((response) => {
      ...
      //还款来源列表
      this.$axios
        .$get('/web/core/dict/findByDictCode/returnSource')
        .then((response) => {
          this.returnSourceList = response.data
        })

      //联系人关系列表
      this.$axios
        .$get('/web/core/dict/findByDictCode/relation')
        .then((response) => {
          this.contactsRelationList = response.data
        })
    },
    ...
    save() {
      ...
      this.$axios
        .$post('/web/core/borrower/auth/save', this.borrower)
        .then((response) => {
          this.active = 2
    ...
    onUploadRemove(file, fileList) {
      ...
      //调用远程文件删除接口
      this.$axios
        .$delete('/web/oss/file/remove?url=' + file.response.data)
        .then((response) => {
          ...
          this.borrower.borrowerAttachList =
            this.borrower.borrowerAttachList.filter(function (item) {
              return item.imageUrl != file.response.data
            })
        })
    },
  },
