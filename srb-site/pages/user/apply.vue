...
export default {
  ...
  methods: {
    getBorrowInfoStatus() {
      this.$axios
        .$get('/web/core/borrowInfo/auth/getBorrowInfoStatus')
        .then((response) => {
          this.borrowInfoStatus = response.data
    ...
    initSelected() {
      //还款方式列表
      this.$axios
        .$get('/web/core/dict/findByDictCode/returnMethod')
        .then((response) => {
          this.returnMethodList = response.data
        })

      //资金用途列表
      this.$axios
        .$get('/web/core/dict/findByDictCode/moneyUse')
        .then((response) => {
          this.moneyUseList = response.data
        })
    },

    getBorrowAmount() {
      this.$axios
        .$get('/web/core/borrowInfo/auth/getBorrowAmount')
        .then((response) => {
          this.borrowAmount = response.data
        })
    },
    ...
    save() {
      this.$axios
        .$post('/web/core/borrowInfo/auth/save', this.borrowInfo)
        .then((response) => {
          this.active = 2
        })
    },
