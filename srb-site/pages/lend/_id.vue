...
export default {
  async asyncData({ $axios, params }) {
    ...
    let response = await $axios.$get('/web/core/lend/show/' + lendId)
    //投资记录
    let responseLendItemList = await $axios.$get(
      '/web/core/lendItem/list/' + lendId
    )
    //还款计划
    let responseLendReturnList = await $axios.$get(
      '/web/core/lendReturn/list/' + lendId
    )

    return {
      lend: response.data.lend, //标的详情
      borrower: response.data.borrower, //借款人信息
      lendItemList: responseLendItemList.data, //投资记录
      lendReturnList: responseLendReturnList.data, //还款计划
    }
  },
  ...
  methods: {
    ...
    fetchAccount() {
      ...
      if (userInfo) {
        this.$axios
          .$get('/web/core/userAccount/auth/getAccount')
          .then((response) => {
    ...
    getInterestCount() {
      this.$axios
        .$get(
          `/web/core/lend/getInterestCount/${this.invest.investAmount}/${this.lend.lendYearRate}/${this.lend.period}/${this.lend.returnMethod}`
        )
        .then((response) => {
          this.interestCount = response.data
        })
    },
        ...
        '前往汇付宝资金托管平台',
        {
          ...
          callback: (action) => {
            ...
            if (action === 'confirm') {
              ...
              this.$axios
                .$post('/web/core/lendItem/auth/commitInvest', this.invest)
                .then((response) => {
    ...
    fetchLendItemReturnList() {
      this.$axios
        .$get('/web/core/lendItemReturn/list/' + this.$route.params.id)
        .then((response) => {
          this.lendItemReturnList = response.data
        })
    },
      ...
      this.$alert(
        ...
        '前往汇付宝资金托管平台',
        {
          ...
          callback: (action) => {
            if (action === 'confirm') {
              this.$axios
                .$post('/web/core/lendReturn/auth/commitReturn/' + lendReturnId)
                .then((response) => {
                  if (response.code == 0) {
                    document.write(response.data)
                  } else {
