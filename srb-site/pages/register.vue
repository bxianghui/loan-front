    ...
    <div v-if="step === 1" class="tdbModule register">
      ...
      <div class="registerCont">
        <ul>
          ...
          <li class="btn">
            <button :disabled="!agree" @click="register()">下一步</button>
          </li>
        </ul>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    send() {
      ...
      let r = mPattern.test(this.userInfo.mobile)
      if (!r) {
      ...
      this.$axios
        .$get('/web/sms/send/' + this.userInfo.mobile)
        .then((response) => {
          if (response.code == 0) {
    ...
    register() {
      ...
      let mPattern = /^[a-zA-Z0-9]{6,24}$/
      let r = mPattern.test(this.userInfo.password)
      if (!r) {
      ...
      this.$axios
        .$post('/web/core/userInfo/register', this.userInfo)
        .then((response) => {
